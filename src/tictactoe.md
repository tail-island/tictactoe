...
今回のコードではあまり有効ではなかった（アルファ・ベータ法による枝刈りが効きすぎた）のですけど、実は今回のコードには関数型プログラミングらしい高速化テクニックを入れています。`getWinner`や`getNextPlayer`、`getNextBoards`のところに書いてある`R.memoize()`です。
