...
今回のコードではあまり有効ではなかった（アルファ・ベータ法による枝刈りが効きすぎた）のですけど、実は今回のコードには関数型プログラミングらしい高速化テクニックを入れているんです。`getWinner`や`getNextPlayer`、`getNextBoards`のところに書いてある`R.memoize()`がそれ。
...
おお、`for`文の「にせんぱーせんと」の実行速度になりました。これなら問題ないですよね？　もし問題があるなら、試合回数を増やすことでいくらでも数字を大きくできますけど……。
